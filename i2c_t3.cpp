...
struct i2cStruct i2c_t3::i2cData[] =
{
    {&I2C0_A1, &I2C0_F, &I2C0_C1, &I2C0_S, &I2C0_D, &I2C0_C2,
     ...
     {}, 0, 0, {}, 0, 0, I2C_OP_MODE_ISR, I2C_MASTER, I2C_PINS_18_19, I2C_STOP, I2C_WAITING,
     0, 0, 0, 0, I2C_DMA_OFF, nullptr, nullptr, nullptr}
#if I2C_BUS_NUM >= 2
   ,{&I2C1_A1, &I2C1_F, &I2C1_C1, &I2C1_S, &I2C1_D, &I2C1_C2,
     &I2C1_FLT, &I2C1_RA, &I2C1_SMB, &I2C1_A2, &I2C1_SLTH, &I2C1_SLTL,
     {}, 0, 0, {}, 0, 0, I2C_OP_MODE_ISR, I2C_MASTER, I2C_PINS_29_30, I2C_STOP, I2C_WAITING,
     0, 0, 0, 0, I2C_DMA_OFF, nullptr, nullptr, nullptr}
#endif
};
                    ...
                    i2c_pins pins, i2c_pullup pullup, i2c_rate rate, i2c_op_mode opMode)
{
    ...
    //
    // This does not call pinConfigure_() because that function alters the opposing pin set (since it is called after
    // begin_() it assumes a pin change), whereas the below only sets up the original pins.
    //
...
uint8_t i2c_t3::setOpMode_(struct i2cStruct* i2c, uint8_t bus, i2c_op_mode opMode)
{
    ...
    *(i2c->S) = I2C_S_IICIF; // clear intr just in case

    // Slaves can only use ISR
    if(i2c->currentMode == I2C_SLAVE) i2c->opMode = I2C_OP_MODE_ISR;
...
uint8_t i2c_t3::setRate_(struct i2cStruct* i2c, uint32_t busFreq, i2c_rate rate)
{
    ...
    if(busFreq == 60000000)
    {
        ...
        default:   ret = 0; *(i2c->F) = 0x00; break;   // 3.0M     20
        }
        *(i2c->FLT) = 4;
    }
    else if(busFreq == 56000000)
    {
        ...
        default:   ret = 0; *(i2c->F) = 0x00; break;   // 2.8M     20 (actual 2.15M)
        }
        *(i2c->FLT) = 4;
    }
    else if(busFreq == 48000000)
    {
        ...
        default:   ret = 0; *(i2c->F) = 0x00; break;   // 2.4M     20 (actual 1.85M)
        }
        *(i2c->FLT) = 4;
    }
    else if(busFreq == 36000000)
    {
        ...
        default:   ret = 0; *(i2c->F) = 0x00; break;   // 1.8M     20
        }
        *(i2c->FLT) = 3;
    }
    else if(busFreq == 24000000)
    {
        ...
        default:   ret = 0; *(i2c->F) = 0x00; break;   // 1.2M     20
        }
        *(i2c->FLT) = 2;
    }
    else if(busFreq == 16000000)
    {
        ...
        default:  ret = 0;  *(i2c->F) = 0x00; break;   // 800k     20
        }
        *(i2c->FLT) = 1;
    }
    else if(busFreq == 8000000)
    {
        ...
        default:  ret = 0;  *(i2c->F) = 0x00; break;   // 400k     20
        }
        *(i2c->FLT) = 1;
    }
    else if(busFreq == 4000000)
    {
        ...
        default:  ret = 0;  *(i2c->F) = 0x00; break;   // 200k     20
        }
        *(i2c->FLT) = 0;
    }
    else if(busFreq == 2000000)
    {
        ...
        default:  ret = 0;  *(i2c->F) = 0x00; break;   // 100k     20
        }
        *(i2c->FLT) = 0;
    }
    else
    ...
    *(i2c->S) = I2C_S_IICIF | I2C_S_ARBL;
    ...
    else if(i2c->opMode == I2C_OP_MODE_ISR || i2c->opMode == I2C_OP_MODE_DMA)
    {
        ...
        if(i2c->opMode == I2C_OP_MODE_DMA && i2c->txBufferLength >= 5) // limit transfers less than 5 bytes to ISR method
        {
            ...
            i2c->DMA->sourceBuffer(&i2c->txBuffer[1],i2c->txBufferLength-2); // DMA sends all except first/last bytes
...
void i2c_t3::sendRequest_(struct i2cStruct* i2c, uint8_t bus, uint8_t addr, size_t len, i2c_stop sendStop, uint32_t timeout)
{
    ...
    if(len == 0 || len > I2C_RX_BUFFER_LENGTH) return;
...
void i2c_isr_handler(struct i2cStruct* i2c, uint8_t bus)
{
    ...
    if(c1 & I2C_C1_MST)
    {
        ...
        if(c1 & I2C_C1_TX)
        {
            if(i2c->activeDMA == I2C_DMA_BULK)
            {
                if(i2c->DMA->complete() && !(*(i2c->S) & I2C_S_TCF))
                {
                ...
                else if(i2c->DMA->complete() && (*(i2c->S) & I2C_S_TCF))
                {
        ...
        else
        {
            ...
            if(i2c->activeDMA == I2C_DMA_BULK)
            {
                if(i2c->DMA->complete() && !(*(i2c->S) & I2C_S_TCF)) // 2nd to last byte
                {
                    // clear DMA interrupt, final byte should trigger another ISR
                    i2c->DMA->clearInterrupt();
                    *(i2c->C1) = I2C_C1_IICEN | I2C_C1_IICIE | I2C_C1_MST | I2C_C1_TXAK; // intr en, Rx mode, DMA disabled, NAK on recv
                }
                else if(i2c->DMA->complete() && (*(i2c->S) & I2C_S_TCF)) // last byte
                {
                    // clear DMA
                    i2c->DMA->clearComplete();
                    i2c->activeDMA = I2C_DMA_OFF;
